...
df <- data.frame(amax, hundred_f, rp_i, nonx_prob_i)

# nonexceedance probability & RP of each year's outflow
# nonx_prob_i <- pGU(amax$outflow, mu = mu, sigma = sigma)
# rp_i <- 1 / (1 - nonx_prob_i)


# 5. plot the 100-year flood magnitude with amax
